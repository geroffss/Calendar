            ...
            db.ref('appointments/' + user.uid).orderByChild('date').equalTo(dateStringForFirebase).once('value').then(function (snapshot) {
              ...
              firebase.auth().onAuthStateChanged(function (user) {
                if (user) {
                  firebase.database().ref('preferences/' + user.uid).once('value').then(function(snapshot) {
                    ...
                    for (var i = startHour; i < endHour; i++) {
                      ...
                      listItem.className = 'border p-5 mt-2 flex justify-between items-center timeDiv rounded-md timeDiv cursor-pointer';
                      listItem.id = 'timeDiv';
                ...
                listItem.addEventListener('click', function (event) {
                  ...
                  var appointmentFormModal = document.getElementById('newAppointmentForm');
                  if (appointmentFormModal) {
                    appointmentFormModal.classList.remove('hidden');
                    document.getElementById('appSelectedHour').innerText = selectedHour;
                  } else {
